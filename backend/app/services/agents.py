...
# ============================================================================
# PLOTLY QUERY ROUTER SIGNATURES & MODULE
# ============================================================================
...
class FigEditorSignature(dspy.Signature):
    ...
    - When calculating mean, median, or other numeric operations, ALWAYS:
      * Use `ensure_numeric(column)` to convert the column first, OR
      * Filter to numeric columns: `numeric_df = df.select_dtypes(include=[np.number])`
      * This handles boolean columns, string numbers, and other non-numeric types safely
    ...
    - For mean calculations: `mean_value = ensure_numeric(df['column']).mean()` or use `df.select_dtypes(include=[np.number])`
    - ALWAYS print the final answer(s) so the user sees the result (e.g., print("Mean price:", value)).
    """
...
class PlotlyQueryRouter(dspy.Module):
    ...
    def __init__(self):
        super().__init__()
        self.plotly_editor_mod = dspy.asyncify(dspy.Predict(PlotlyEditorSignature))
        self.fig_editor_mod = dspy.asyncify(dspy.Predict(FigEditorSignature))
        self.general_qa_mod = dspy.asyncify(dspy.Predict(GeneralQASignature))
        self.router = dspy.asyncify(dspy.Predict(RouterSignature))
